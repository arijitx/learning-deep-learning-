...
class DataLoader():
	def __init__(self,q_path,s_path,w2id_path,batch_size=64):
		...
		slen=[]
		for i in range(self.sentence.shape[0]):
			slen.append(self.question[i].shape[0])
		idx=np.array(slen).argsort()

		self.sentence=self.sentence[idx]
		self.question=self.question[idx]

		print('Known Words :',len(self.known_words))
		print('Unknown Words :',len(self.unknown_words))
		self.idx=0
